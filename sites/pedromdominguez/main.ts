...
// Import DenoGenesis Framework - Using centralized mod.ts
import {
...
try {
  ...
  if (isValid) {
    console.log("\x1b[32m%s\x1b[0m", "✅ Framework integrity validated");
  } else {
    console.log("\x1b[33m%s\x1b[0m", "⚠️ Framework integrity check returned warnings");
  }
} catch (error) {
  console.log("\x1b[31m%s\x1b[0m", `❌ Framework integrity check failed: ${error.message}`);
}
...
console.log("\x1b[32m%s\x1b[0m", `         📦 Framework Source: Centralized mod.ts`);
console.log("\x1b[35m%s\x1b[0m", "✨========================================================✨");
...
const serverInfo = [
  ...
  { label: 'Framework Source', value: 'Centralized mod.ts' }
];
...
setTimeout(() => {
  ...
  console.log("\x1b[32m%s\x1b[0m", `   📦 Framework: Centralized mod.ts`);
