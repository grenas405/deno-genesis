...
import { SiteSettingsService } from "../services/settingsService.ts";
import { SiteSettingsInput } from "../types/settings.d.ts";
...
export const getSiteSettings = async (ctx: Context) => {
  try {
    ...
    if (!settings) {
      console.warn(
        `⚠️ No settings found for site key: ${Deno.env.get("SITE_KEY")}`,
      );
      ctx.response.status = 404;
      ctx.response.body = {
        error: `Settings not found for ${Deno.env.get("SITE_KEY")}`,
      };
      return;
    }
...
export const updateSiteSettings = async (ctx: Context) => {
  try {
    ...
    ctx.response.status = 200;
    ctx.response.body = {
      message: `Settings updated for ${Deno.env.get("SITE_KEY")}`,
    };
  } catch (err) {
