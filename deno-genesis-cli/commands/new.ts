...
const INDUSTRIES: IndustryDefinition[] = [
  ...
  },
  {
    ...
    defaultColorSchemes: ["corporate-navy", "trust-burgundy", "professional-gray"],
    templateFeatures: [
      "case-studies",
      "team-expertise",
      "credentials-display",
  ...
  },
  {
    ...
    defaultColorSchemes: ["retail-vibrant", "ecommerce-clean", "boutique-elegant"],
    templateFeatures: [
      "product-catalog",
      "shopping-cart",
      "inventory-display",
...
`);
    ...
    if (!siteExists) {
      ...
      console.error(`   Run 'genesis init' first to create the site structure.`);
      return 1;
    }
...
async function selectSiteDirectory(context: CLIContext): Promise<string | null> {
...
async function gatherBusinessInformation(context: CLIContext): Promise<BusinessInfo> {
...
async function selectColorScheme(industryId: string): Promise<ColorScheme> {
  ...
  console.log(`${industry.defaultColorSchemes.length + 1}. Browse all color schemes`);
...
function validateBusinessInfo(info: BusinessInfo): { valid: boolean; error?: string } {
...
function generateMetaDescription(info: BusinessInfo): string {
  return `${info.name} - Professional ${info.businessType} services in ${info.address.city}, ${info.address.state}. Contact us at ${formatPhoneDisplay(info.phone)}.`;
}
...
function generateHeroSubheading(info: BusinessInfo): string {
  const templates: Record<string, string> = {
    construction: `Professional ${info.businessType} serving ${info.address.city}`,
    healthcare: `Your trusted ${info.businessType} in ${info.address.city}`,
    "professional-services": `Expert ${info.businessType} for ${info.address.city} and surrounding areas`,
    ...
    "food-beverage": `Experience exceptional ${info.businessType} in ${info.address.city}`
  };
        ...
        <section class="hero" aria-label="Hero section">
            <div class="container">
                <div class="hero-content">
                    ...
                    <p class="hero-subheading">${generateHeroSubheading(businessInfo)}</p>
                    ...
                    <div class="hero-contact" id="contact-info">
                        ...
                        <div class="contact-item">
                            <a href="${generateGoogleMapsURL(businessInfo.address)}" class="contact-link address-link" target="_blank" rel="noopener" aria-label="View ${businessInfo.name} location on map">
        ...
        <section class="section" id="contact" aria-label="Contact information">
            <div class="container">
                ...
                <div class="contact-info">
                    <div class="contact-item">
                        <h3>📞 Phone</h3>
                        <a href="tel:${businessInfo.phone}">${formatPhoneDisplay(businessInfo.phone)}</a>
                    </div>
                    <div class="contact-item">
                        <h3>✉️ Email</h3>
                    ...
                    <div class="contact-item">
                        ...
                        <a href="${generateGoogleMapsURL(businessInfo.address)}" target="_blank" rel="noopener">View on Map</a>
                    </div>
                </div>
            </div>
        </section>
    ...
    <footer role="contentinfo">
        <div class="container">
            <div class="footer-content">
                ...
                <div class="footer-section">
                    <h4>Contact</h4>
                    <p>Phone: <a href="tel:${businessInfo.phone}">${formatPhoneDisplay(businessInfo.phone)}</a></p>
...
export default newCommand;#!/usr/bin/env -S deno run --allow-read --allow-write --allow-net
 ...
 * - Accept text input: User prompts for business information
 * - Produce text output: HTML, CSS, JS, and configuration files
 * - Filter and transform: Business data → Professional website
...
`);
    ...
    // Step 1: Determine sites directory
    const sitesDirectory = await selectSitesDirectory(context);
    if (!sitesDirectory) {
      console.error("❌ No sites directory selected. Exiting.");
      return 1;
    }
    ...
    // Step 4: Generate site structure
    const siteName = generateSiteName(businessInfo.name);
    const siteDirectory = join(sitesDirectory, siteName);

    // Check if site already exists
    if (await exists(siteDirectory)) {
      console.error(`❌ Site directory already exists: ${siteDirectory}`);
      console.error(`   Please choose a different business name or remove the existing directory.`);
      return 1;
    }
...
📖 Documentation: ${siteName}/README.md
`);
...
async function selectSitesDirectory(context: CLIContext): Promise<string | null> {
  const defaultSitesDir = join(context.cwd, "sites");
  const sitesExists = await exists(defaultSitesDir);

  if (sitesExists) {
    console.log(`\n📁 Found sites directory: ${defaultSitesDir}`);
    const useDefault = await promptYesNo("Use this directory?", true);
  ...
  console.log("\n📂 Choose sites directory:");
  console.log("1. Create default ./sites directory");
  console.log("2. Specify custom path");
  console.log("3. Cancel");

  const choice = await promptChoice([1, 2, 3]);

  switch (choice) {
    case 1:
      await ensureDir(defaultSitesDir);
      return defaultSitesDir;
    
    case 2:
      const customPath = await promptText("Enter path:", { required: true });
      if (customPath) {
        await ensureDir(customPath);
        return customPath;
      }
      return null;
...
async function promptChoice(choices: number[]): Promise<number> {
  while (true) {
    ...
    if (isNaN(choice) || !choices.includes(choice)) {
...
async function generateFrontend(
  ...
  await createSiteDirectories(siteDirectory);
  await generateSiteConfig(businessInfo, siteDirectory);
  await generateHTMLPages(businessInfo, siteDirectory);
  await generateStylesheets(businessInfo, siteDirectory);
  await generateScripts(businessInfo, siteDirectory);
  await generateDocumentation(businessInfo, siteDirectory);
  await createCoreSymlinks(siteDirectory, context);

  console.log("✅ All files generated successfully!");
}

async function createSiteDirectories(siteDirectory: string): Promise<void> {
  const directories = [
    siteDirectory,
    join(siteDirectory, "public"),
    join(siteDirectory, "public/pages"),
...
async function generateSiteConfig(
  ...
  const config = `// ${businessInfo.name} - Site Configuration
// Generated by: genesis new
// Date: ${new Date().toISOString()}
...
export const siteConfig: SiteConfig = {
  ...
  colorScheme: {
    name: "${businessInfo.colorScheme.name}",
    primaryDark: "${businessInfo.colorScheme.primaryDark}",
    primaryMedium: "${businessInfo.colorScheme.primaryMedium}",
    primaryLight: "${businessInfo..colorScheme.accentPrimary}",
    accentSecondary: "${businessInfo.colorScheme.accentSecondary}",
    textPrimary: "${businessInfo.colorScheme.textPrimary}",
    textSecondary: "${businessInfo.colorScheme.textSecondary}",
    textTertiary: "${businessInfo.colorScheme.textTertiary}"
...
`;
  ...
  const configPath = join(siteDirectory, "site.config.ts");
  await Deno.writeTextFile(configPath, config);
        ...
        <section class="section" id="contact" aria-label="Contact information">
            <div class="container">
                ...
                <div class="contact-info">
                    ...
                    <div class="contact-item">
                        <h3>📍 Addre <div class="hero-cta">
                   colorScheme.primass</h3>
                        <p>${formatFullAddress(businessInfo.address)}</p>
                        <a href="${generateGoogleMapsURL(businessInfo.address)}" target="_blank" rel="noopener">View on Map</a>
                    </div>
                </div>
...
:root {
  ...
  --shadow-lg: 0 10px 15px -3px rgba(0, 0, 0, 0.1);
  --shadow-xl: 0 ryLight}",
    accentPrimary20px 25px -5px rgba(0, 0, 0, 0.1);
...
async function generateDocumentation(
  ...
  const readme = `# ${businessInfo.name}

> Professional website built with Deno Genesis Framework
...
Business information in \`site.config.ts\`:
- Contact: ${formatPhoneDisplay(businessInfo.phone)}, ${businessInfo.email}
- Address: ${formatFullAddress(businessInfo.address)}
- Colors: ${businessInfo.colorScheme.name}
...
### Add Pages
Create new directories in \`public/pages/\`

### Modify Content
Edit HTML files in \`public/pages/\`

## 🚀 Deployment

\`\`\`bash
genesis deploy yourdomain.com --port 3000
\`\`\`

## 📞 Contact

**${businessInfo.name}**
- Phone: ${formatPhoneDisplay(businessInfo.phone)}
- Email: ${businessInfo.email}
- Address: ${formatFullAddress(businessInfo.address)}

---

Built with [Deno Genesis Framework](https://github.com/grenas405/deno-genesis)  
Generated: ${new Date().toISOString()}
`;

  const readmePath = join(siteDirectory, "README.md");
...
async function createCoreSymlinks(
  ...
  if (!await exists(coreDir)) {
    console.log("  ⚠️  Core directory not found - skipping symlinks");
    console.log("     Run 'genesis init' to set up the core framework");
    return;
  }
  ...
  for (const link of symlinks) {
    const targetPath = join(coreDir, link);
    const linkPath = join(siteDirectory, link);

    if (!await exists(targetPath)) {
      if (context.verbose) {
        console.log(`  ⚠️  Skipped: ${link} (target not found)`);
      }
      continue;
    }

    try {
      const relativePath = relative(siteDirectory, targetPath);
      ...
      if (context.verbose) {
        console.log(`  ⚠️  Failed to link ${link}: ${error.message}`);
      }
    }
  }

  if (linkedCount > 0) {
    console.log(`  ✅ Created ${linkedCount} core framework symlinks`);
  }
}
...
export default newCommand;: "${businessInfo
